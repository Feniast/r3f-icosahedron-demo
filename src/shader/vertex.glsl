...
void main() {
  ...
  //   position.x + sin(position.y + uTime * 0.1) + uTime * 0.07, 
  //   position.y + cos(position.x + uTime * 0.12), 
  //   position.z + 0.15 * uTime)
  // );
  float noise = snoise(vec3(
    vNormal.x + sin(vNormal.y + uTime * 0.1) + uTime * 0.05, 
    vNormal.y + cos(vNormal.x + uTime * 0.12) + uTime * 0.05, 
    vNormal.z + 0.09 * uTime)
  );
  vec3 pos = position * (1. + noise * uDistortFactor);
